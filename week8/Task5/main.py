...
這邊要做一個個人資訊頁面
"""
@app.get("/user_data_edit", response_class=HTMLResponse)
def user_data_edit(request: Request, message: str):
    return templates.TemplateResponse("user_data.html", {"request": request, "message": message})



@app.get("/api/member")
def only_myself_message(request: Request, input: str=''):
    ...
    if "user_id" not in request.session: #如果user_id沒有在session中就等回首頁
        ...
        cursor = website_db.cursor()
        cursor.execute("select message from message where username=%s;", (user_id,))
        myresult = cursor.fetchall()#回傳所有資料庫指令結果
        # user_name = myresult[0][0]
        user_message = myresult[0][0]
